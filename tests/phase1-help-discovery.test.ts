...
`)
  ...
  describe('validate-to-measure hint', () => {
    let logs: string[]
    let consoleSpy: ReturnType<typeof vi.spyOn>
    let consoleErrorSpy: ReturnType<typeof vi.spyOn>
    ...
    it('should include a measure hint in structured validate output', async () => {
...
`)
      ...
      await program.parseAsync(
        ['node', 'aixbt', '--format', 'json', 'recipe', 'validate', recipeFile],
        { from: 'node' },
      )
      ...
      expect(parsed.hint).toContain('recipe measure')
    })

    it('should include a measure hint in human-format validate output', async () => {
      const recipeFile = join(tempDir, 'valid.yaml')
      writeFileSync(recipeFile, `
name: test-recipe
version: "1.0"
...
`)
      ...
      program.exitOverride()
      await program.parseAsync(
        ['node', 'aixbt', 'recipe', 'validate', recipeFile],
        { from: 'node' },
      )

      const allOutput = stripAnsi(logs.join('\n'))
      expect(allOutput).toContain('recipe measure')
    })
  })
})
